...
from src.data.dataset import CelebA
from src.pipeline.composable_stable_diffusion_pipeline_noise import ComposableStableDiffusionPipeline
...
def log_visualization(
    ...
    for step, batch in enumerate(vis_dataloader):
        pixel_values = batch[0].permute(0, 3, 1, 2).to(device=accelerator.device, dtype=weight_dtype)
        label = batch[1].to(device=accelerator.device, dtype=weight_dtype)
        ...
        with torch.autocast("cuda"):
            ...
            images_gen = torch.cat(images_gen, dim=1)
        ...
        grid_image = torch.cat(
            [pixel_values.unsqueeze(1).clamp(0, 1), images_gen.clamp(0, 1), images_recon.unsqueeze(1).clamp(0, 1)], dim=1)
...
def main(args):
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            for model in models:
                ...
                # continue if not one of [UNet2DConditionModelWithPos]
    ...
    train_dataset = CelebA(args.dataset_root)

    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, num_workers=8, batch_size=args.train_batch_size, prefetch_factor = 8, pin_memory=True, persistent_workers=True,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            text_emb_batch = torch.flatten(label, 0, 1) #(B*3)*77*1024
            ...
            # conposed predictions
            composed_pred = cond_pred.mean(dim=1)
            ...
            if (accumulate_steps + 1) % args.gradient_accumulation_steps == 0:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.validation_steps == 0:
                        print(label[:args.num_validation_images])
                        num_digits = len(str(args.max_train_steps))
                        folder_name = f"image_logging_{global_step:0{num_digits}}"
                        image_log_dir = os.path.join(accelerator.logging_dir, folder_name, )
                        os.makedirs(image_log_dir, exist_ok=True)
                        ...
                        vis_output =cond_pred[:args.num_validation_images]/vae.config.scaling_factor
                        vis_comps = []
                        ...
                        grid_image = torch.cat(
                            [vis_pix.unsqueeze(1), vis_comps.clamp(0, 1).to(torch.float32), vis_recon.clamp(0, 1).to(torch.float32).unsqueeze(1)], dim=1)
                        grid_image = make_grid(
